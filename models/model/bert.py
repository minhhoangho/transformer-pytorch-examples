...
def create_model():
    ## BERT encoder
    # Download model from https://huggingface.co/bert-base-uncased
    encoder = TFBertModel.from_pretrained("./data/bert-base-uncased", output_attentions=True)
    n_hidden = 2
    rnn_size = max_len
    output_dim = 768 // 2
    # output_dim = 32
    ...
    input_ids = Input(shape=(max_len,), dtype=tf.int32, name="input_id")
    token_type_ids = Input(shape=(max_len,), dtype=tf.int32, name="token_type_id")
    ...
    lstm = LSTM(units=output_dim, unit_forget_bias=True, name="lstm", return_sequences=True, dropout=0.2,
                recurrent_dropout=0.2)
    # embedding = tf.transpose(embedding, perm=[1, 0, 2])
    # embedding = tf.cast(embedding, dtype=tf.int32)
    bidirectional = Bidirectional(lstm)(embedding)
    lstm2 = LSTM(units=output_dim, return_sequences=True, dropout=0.5, recurrent_dropout=0.5)(bidirectional)
    # output = Dense(output_dim, activation="relu")(bidirectional)
    # output = TimeDistributed(Dense(n_tags, activation="softmax"), name="output")(output)
    output = TimeDistributed(Dense(64, activation="tanh"), name="output")(lstm2)
    crf = CRF(n_tags, name="crf_layer")  # CRF layer
    ...
    # loss = crf_log_likelihood(sequence_lengths=max_len, inputs=output, tag)
    optimizer = keras.optimizers.Adam(lr=5e-5)

    # model.compile(optimizer=optimizer, loss=loss, metrics=['accuracy'])
    # model.compile(optimizer="rmsprop", loss=create_joined_crf_loss(crf), metrics=[crf_viterbi_accuracy])
    # model.compile(optimizer=optimizer, loss=create_joined_crf_loss(crf), metrics=[crf_viterbi_accuracy])
    model.compile(optimizer=optimizer, loss=crf.get_loss, metrics=[crf.get_accuracy])
    return model
